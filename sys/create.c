	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	pptr->Offset = 0;
        for(i=0;i<TOTAL_BACKING_STORES;i++){
                pptr->BackingStores[i].bs_status = UNMAPPED;
                pptr->BackingStores[i].bs_pid = -1;
                pptr->BackingStores[i].bs_vpno = -1;
                pptr->BackingStores[i].bs_npages = 0;
                pptr->BackingStores[i].bs_sem = -1;
                pptr->BackingStores[i].bs_PrivateHeap = 0;
        }
        if(create_Page_Directory(pid) == SYSERR){
                kprintf("\nError while creating Page Directory");
                restore(ps);
                return SYSERR;
        }
	restore(ps);
