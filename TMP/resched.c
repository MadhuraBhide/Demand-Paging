...
#include <q.h>
#include <paging.h>
void write_back_data(int pid);
void read_data(int pid);
...
void write_back_data(int pid){
	int i,store,pageth;
	//Find dirty pages for the old process(optr) and write to BS
	for(i=0;i<NFRAMES;i++){
		if(frm_tab[i].fr_status == MAPPED && frm_tab[i].fr_type == FR_PAGE && frm_tab[i].fr_pid == pid){
			if(bsm_lookup(pid, (frm_tab[i].fr_vpno)*NBPG, &store, &pageth) != SYSERR){
				char *src = (FRAME0+i)*NBPG;
				pd_t *pd = proctab[pid].pdbr + 4*(frm_tab[i].fr_vpno >> 10);
...
void read_data(int pid){
	//Read all the mapped pages from BS for new process(nptr)
	int i,store,pageth;
        for(i=0;i<NFRAMES;i++){
                if(frm_tab[i].fr_status == MAPPED && frm_tab[i].fr_type == FR_PAGE && frm_tab[i].fr_pid == pid){
                        if(bsm_lookup(pid, (frm_tab[i].fr_vpno)*NBPG, &store, &pageth) != SYSERR){
                        char *dst = (FRAME0+i)*NBPG;
