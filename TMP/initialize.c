...
#endif
	//Initialize frame and BS structures
	init_bsm();
	init_frm();
	//Set Page fault interrupt
	set_evec(14,pfintr);
	//Initialize global page tables 
	if(Initialize_global_PageTables() == SYSERR) return SYSERR;
	//Create page directory for Null process
	if(create_Page_Directory(NULLPROC) == SYSERR) return SYSERR;
        write_cr3(proctab[NULLPROC].pdbr);
	//Enable paging
	enable_paging();
 	//Initialize data structure for page replacement
	SChead = SCtail = clk_ptr = NULL;	
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->Offset = 0;
        int k;
        for(k=0;k<TOTAL_BACKING_STORES;k++){
                pptr->BackingStores[k].bs_status = UNMAPPED;
                pptr->BackingStores[k].bs_pid = -1;
                pptr->BackingStores[k].bs_vpno = -1;
                pptr->BackingStores[k].bs_npages = 0;
                pptr->BackingStores[k].bs_sem = -1;
                pptr->BackingStores[k].bs_PrivateHeap = 0;
        }
